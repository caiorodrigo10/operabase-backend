...
export class MinimalStorage implements IStorage {
  ...
  async getAppointmentsByDateRange(startDate: Date, endDate: Date): Promise<any[]> {
    try {
      // ULTRA SIMPLE: Get all appointments and filter manually
      ...
      // Manual filtering in JavaScript for maximum reliability
      const filtered = data?.filter((apt: any) => {
        ...
        // Check if appointment overlaps with our time range
        return aptStart < endDate && aptEnd > startDate;
      }) || [];
      
      console.log(`🔍 ULTRA SIMPLE: Found ${filtered.length} overlapping appointments out of ${data?.length || 0} total`);
      filtered.forEach((apt: any) => {
        console.log(`📋 OVERLAP: ${apt.id} - ${apt.scheduled_date} (${apt.duration_minutes}min)`);
      });
