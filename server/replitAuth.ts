...
async function upsertUser(
  ...
  // Check if user exists, if not create with first available clinic
  let user = await storage.getUserByEmail(claims["email"]);
  
  if (!user) {
    // Create new user - they'll be assigned to a clinic during onboarding
...
export const hasClinicAccess = (paramName: string = 'clinicId'): RequestHandler => {
  return async (req, res, next) => {
    ...
    const hasAccess = await storage.userHasClinicAccess(user.dbUser.id, clinicId);
    if (!hasAccess) {
