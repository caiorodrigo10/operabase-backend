...
async function createSupabaseTables() {
  ...
  // Executar statements de criação de tabelas
  for (const sql of sqlStatements) {
    try {
      const { error } = await supabaseAdmin.rpc('exec_sql', { query: sql });
      if (error) {
        console.warn(`⚠️ ${sql.substring(0, 30)}...`);
      } else {
        successCount++;
      }
    } catch (error) {
  ...
  for (const sql of indexStatements) {
    try {
      ...
      // Índices podem falhar se já existirem
    }
  }

  console.log(`✅ Criação concluída: ${successCount} comandos executados`);
  ...
  if (error && error.code === '42P01') {
    console.log('❌ Tabelas não foram criadas - usando SQL do arquivo');
    return false;
  } else {
...
async function main() {
  try {
    ...
    if (success) {
      console.log('\n🎉 Próximo passo: execute a migração de dados');
      console.log('tsx migrate-data-only.ts');
    } else {
      console.log('\n📝 Execute manualmente o SQL no painel do Supabase');
      console.log('Arquivo: schema-supabase.sql');
    }
  } catch (error) {
